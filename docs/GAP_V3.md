...
| 5 | The 21 named joins | 2 | **Declared** with their `feeds` in `joins.py`; `run_join` works; the per-join data adapters are not written, so **no join has ever run against the workbook** — the largest untested surface in the repo. `CLAUDE.md` also requires joins to run once before rules and be cached on the context; there is no join phase in the runner yet |
