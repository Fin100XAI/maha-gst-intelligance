...
| 5 | The 21 named joins | 2 | **Join phase built.** `RuleContext.joins` runs all 21 once and caches them; adapters exist for **J03, J04, J07** and the other 18 report their missing sheet by name. First run on the reference workbook: J03 pairs 4,404 of 4,500 at L1 with 125 documents in 2A and not in 2B; J04 finds 8 defaulting invoices with no 2B counterpart; all three reconcile. J07 does not yet implement digit containment, so X-02 still matches directly (D-0090) |
