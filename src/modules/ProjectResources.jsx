  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 mb-4">
        ...
        <KpiCard label={t('Judicial authorities')} value={S.judicial} unit={t('verified against reports')} tone="amber" icon={Gavel} />
        <KpiCard label={t('Official data sources')} value={S.official} unit={t('government published')} tone="green" icon={Globe} />
        <KpiCard label={t('Methods')} value={S.methods} unit={t('each with its failure mode')} tone="steel" icon={Sigma} />
        <KpiCard label={t('Software packages')} value={S.software} unit={t('all open-source')} tone="steel" icon={Package} />
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card title={t('Simulated')} subtitle={t('Generated from a fixed seed. None of it describes a real taxpayer.')}>
        ...
        <Card title={t('Real')} subtitle={t('Verified against published sources on {0}.', D.retrievedOn)}>
          <ul className="space-y-2">
            {D.real.map((s, i) => (
              <li key={i} className="flex items-start gap-2 text-[12.5px] text-navy-800 leading-relaxed">
...
function LegalView() {
  return (
    <div className="space-y-4">
      <Card title={t('Statute and subordinate legislation')} subtitle={t('Encoded as computation rather than summarised. The limitation engine computes from these, which is why its output can go into a notice.')} padded={false}>
        <div className="divide-y divide-steel-100">
          {LEGAL_SOURCES.map(s => <SourceRow key={s.id} s={s} icon={Scale} />)}
        </div>
      </Card>

      <Card title={t('Judicial authority')} subtitle={t('Verified against published reports. Where only a holding could be confirmed, the case name is left blank rather than invented.')} padded={false}>
        <div className="divide-y divide-steel-100">
...
function DataView() {
  return (
    <div className="space-y-4">
      <Card title={t('Official published sources')} padded={false}>
        <div className="divide-y divide-steel-100">
      ...
      {DATASET_POINTER_LIST.length > 0 && (
        <Card
          ...
          subtitle={t('Recorded so the department knows they exist, with nothing inferred from them — the endpoints returned HTTP 403 when fetched, and guessing at their contents would have been worse than leaving them empty.')}
        >
...
function MethodView() {
  return (
    <Card
      ...
      subtitle={t('Each with the reason it was chosen and the way it fails. A method whose failure mode is not stated is a method nobody can audit.')}
      padded={false}
    >
      <div className="divide-y divide-steel-100">
        {METHODS.map(m => (
          <div key={m.id} className="px-5 py-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 ml-5.5">
              ...
              <div className="rounded-lg border border-red-200 bg-red-50/40 px-3 py-2">
                ...
                <p className="text-[11.5px] text-navy-800 leading-relaxed">{m.failure}</p>
              </div>
            </div>
          </div>
        ))}
...
function SoftwareView() {
  return (
    <div className="space-y-4">
      <Card title={t('Software')} subtitle={t('All open-source. The platform makes no call to the open internet for its figures, models or maps, and runs entirely within the department’s own infrastructure.')} padded={false}>
        <div className="divide-y divide-steel-100">
          {SOFTWARE.map(s => (
...
function SourceRow({ s, icon: Icon }) {
  return (
