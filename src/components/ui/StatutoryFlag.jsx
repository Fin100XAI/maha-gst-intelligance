          ...
          <>
            ...
            <div className="flex flex-wrap gap-1.5">
              {review.barred.slice(0, 8).map(b => (
                <span key={b.gstin} title={b.verdict} className="inline-flex items-center gap-1 rounded-md border border-red-200 bg-white text-[11px] px-2 py-0.5">
...
export function StatutoryVerdict({ gstin }) {
  ...
  return (
    <div className={`rounded-lg border px-3.5 py-2.5 mb-3 ${severe ? 'border-red-300 bg-red-50/60' : 'border-amber-300 bg-amber-50/60'}`}>
      ...
      <p className="text-[12px] text-navy-800 leading-relaxed">{pos.verdict}</p>
      {severe && (
