...
function Item({ to, label, hint }: { to: string; label: string; hint?: string }): JSX.Element {
  return (
    <li>
      <NavLink
        to={to}
      ...
      <div className="mx-auto flex max-w-[1240px] gap-8 px-6 py-8">
        <nav aria-label="Sections" className="w-60 shrink-0">
          ...
          <Rail title="Scrutiny">
            <ul className="space-y-0.5">
              <Item
                to={`/scrutiny/portfolio${suffix}`}
                label="Portfolio"
                hint="Who needs attention"
              />
              <Item
                to={`/scrutiny/matrix${suffix}`}
                label="The 141 checks"
                hint="The department's own matrix"
              />
              <Item
